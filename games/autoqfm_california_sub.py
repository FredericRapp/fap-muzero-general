        ...
        if "(x)" not in self.fm_str:
            ...
            if var_kernel < 0.001:
                ...
                if pred_error <= (self.best_pred_error-0.01) and pred_error <= (self.last_pred_error-0.01) and self.last_pred_error != 0.0:
                    ...
                    #     reward_overall_best = 1000.0  # You can adjust the reward value
...
class Game(AbstractGame):
    ...
    def reset(self):
        ...
        """
        self.storage_dict["best_fm_str"].append(self.env.storage_dict["best_fm_str"])
        self.storage_dict["best_pred_error"].append(self.env.storage_dict["best_pred_error"])
        print("storage dict of the game / agent at reset:", self.storage_dict)
        print("storage dict of the environment:", self.env.storage_dict)
