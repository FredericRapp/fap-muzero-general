...
class KernelPropertiyEnv(gym.Env):
    def __init__(self, training_data, training_labels, num_qubits, num_features):
        ...
        print('Lets goooo PQK AutoQFM CALI!')
...
class MuZeroConfig:
    def __init__(self):
        ...
        #     os.makedirs(logdir)
        ### Training
        self.results_path = pathlib.Path(logdir).resolve().parents[1] / "autoqfm_run_1_california/results" / pathlib.Path(logdir).stem / datetime.datetime.now().strftime("%Y-%m-%d--%H-%M-%S")  # Path to store the model weights and TensorBoard logs
